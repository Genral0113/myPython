...
def reward_function(params):
    ...
    # car_actions = ['speed_up', 'speed_up', 'slow_down', 'speed_up', 'speed_up', 'slow_down', 'speed_up', 'slow_down', 'speed_up']
    ...
    for i in range(len(start_waypoints) - 1):
        if start_waypoints[i] <= closest_waypoints[0] < start_waypoints[i + 1]:
            ...
            target_point_x = (waypoints_inn[end_waypoint][0] + waypoints[end_waypoint][0]) * 0.5
            target_point_y = (waypoints_inn[end_waypoint][1] + waypoints[end_waypoint][1]) * 0.5
            target_directions = directions_of_2points([x, y], [target_point_x, target_point_y])

            heading_diff = abs(target_directions - heading)
            if heading_diff > 180:
    ...
    if track_width * 0.25 <= distance_from_center < track_width * 0.5:
        if heading_diff < 15:
            if car_action == 'speed_up':
                if throttle > fast_throttle:
                    reward = throttle
